...
const { assets } = require('../db/schema');

/**
 * 素材仓储。强制 user_id 隔离;本体在 MinIO,这里存 object_key/mime/size。
 */
function createAssetsRepo(deps) {
  const table = deps.table || assets;
  const { db } = deps;
  ...
  /** 按 (user_id, content_hash) 找已存在的同内容素材,用于去重。空 hash 直接返回 null。 */
...
module.exports = { createAssetsRepo };
