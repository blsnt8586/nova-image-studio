...
#
# 加速:全部 npm install 用 BuildKit 缓存挂载(--mount=type=cache),
# npm 下载缓存持久保存在宿主机,跨构建复用 —— 即使依赖层失效也只装不下载。
...
# Next 构建缓存挂载:.next/cache 跨构建复用,只改代码时增量编译更快。
...
RUN --mount=type=cache,target=/root/.npm npm install --no-audit --no-fund --include=dev drizzle-kit@^0.30.0
